...
from LexPOSBartModel import LexPOSBartModel
from transformers import BartTokenizer
from scipy import spatial
from sent2vec.vectorizer import Vectorizer

class LexPOSBartForConditionalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = LexPOSBartModel(config)  # custom BartModel
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # Add cosine distance between labels and predictions to loss  
            decoded_labels = []
            decoded_logits = []
